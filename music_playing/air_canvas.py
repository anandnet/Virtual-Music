...
kick = mixer.Sound(os.path.join(PATH, "untitled_a_kick_01.wav"))
hhat = mixer.Sound(os.path.join(PATH, "beachparty_a_hihat_01.wav"))
snare = mixer.Sound(os.path.join(PATH, "beachparty_a_snare_01.wav"))
#sound.loop=  True
...
while True: 
    ...
    if len(cnts_blue) > 0:
        ...
        if int(y_b)+(h//2 - 100) + radius_b > (h//2 + 50+5):
            ...
            if not already_in_blue:
            #if not mixer.music.get_busy():
                kick.play()
            
            already_in_blue = True
            #if sound:
    ...
    if len(cnts_yellow) > 0:
        ...
        if int(y)+(h//2 - 100) + radius > (h//2 + 50+5):
            ...
            if not already_in_yellow:
            #if not mixer.music.get_busy():
                snare.play()
            
            already_in_yellow = True
            #if sound:
                #print("Sound AVailable")
            #    print(sound.state)
            #    sound.play()
